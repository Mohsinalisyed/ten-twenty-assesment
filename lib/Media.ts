...
import { GL, MediaProps, ScreenSize, Viewport } from "@/types";
...
export class Media {
  ...
  update(scroll: { current: number; last: number }, direction: "right" | "left") {
    ...
    if (this.bend === 0) {
      ...
      const bendScale = this.screen.width <= 768 ? 0.5 : 1; // mobile adjustment
      if (this.bend > 0) {
        this.plane.position.y = -arc * bendScale;
        this.plane.rotation.z = -Math.sign(x) * Math.asin(effectiveX / R) * bendScale;
      } else {
        this.plane.position.y = arc * bendScale;
        this.plane.rotation.z = Math.sign(x) * Math.asin(effectiveX / R) * bendScale;
      }
  ...
  onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
    ...
    if (this.title.update) this.title.update({ scale: fontScale });
  }
}
